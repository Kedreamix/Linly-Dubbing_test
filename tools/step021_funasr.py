...
def transcribe_audio(folder,  device='auto', batch_size=2):
    if os.path.exists(os.path.join(folder, 'transcript_funasr.json')):
    ...
    with open(os.path.join(folder, 'transcript_funasr.json'), 'w', encoding='utf-8') as f:
        json.dump(transcript, f, indent=4, ensure_ascii=False)
    logger.info(f'Transcribed {wav_path} successfully, and saved to {os.path.join(folder, "transcript_funasr.json")}')
...
def transcribe_all_audio_under_folder(folder, device='auto', batch_size=32):
    for root, dirs, files in os.walk(folder):
        if 'audio_vocals.wav' in files and 'transcript_funasr.json' not in files:

...
elif model_name == 'Qwen/Qwen1.5-4B-Chat':
...
import torch
def release_memory():
    # Manually free up unused GPU memory
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        print("Released unused GPU memory.")
...
def summarize(info, transcript, target_language='简体中文'):
    ...
    base_url=os.getenv('OPENAI_API_BASE', 'https://api.xiaoai.plus/v1'),
...
if __name__ == '__main__':
    translate_all_transcript_under_folder(
        r'/root/YouDub-webui/videos/黑纹白斑马/20240805 中配什么是引力波Amber L Stuver - TED-Ed', '简体中文')